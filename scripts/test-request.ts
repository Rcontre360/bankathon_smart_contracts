...
async function main() {
  ...
  const config = JSON.parse(fs.readFileSync(".airnode-starter.config.json") as any);
  console.log("Using config: " + JSON.stringify(config, null, 2));

  // Get the preconnected wallet from Hardhat
  const [wallet] = await ethers.getSigners();
  console.log(
    `Using wallet ${wallet.address} and Airnode ${config.airnodeContractAddress}`
  );

  // Get an instance of the ExampleClient we deployed in setup.js
  const exampleClient = await ethers.getContractAt(
    "ExampleClient",
    config.exampleClientAddress,
    wallet
  );
  ...
  const airnode = new ethers.Contract(
    ...
    (airnodeProtocol as any).AirnodeArtifact.abi,
    wallet
  );

  // Derive the designated wallet address
  const designatedWalletAddress = await deriveDesignatedWallet(
    airnode,
    apiProviderId,
  ...
  const designatedWalletBalance = await ethers.provider.getBalance(
    designatedWalletAddress
  );
  if ((designatedWalletBalance as any) >= 1e14) {
    // >= 0.0001 RBTC
    console.log(
      `Designated wallet ${designatedWalletAddress} has ${weiToEth(
  ...
  console.log(
    `Making the request to API provider ${apiProviderId} endpoint ${endpointId}...`
  );
  console.log("Request params: " + JSON.stringify(requestParams, null, 2));

  const requestId = await makeRequest();
  console.log(
    `Completed the request with ID ${requestId}, waiting for fulfillment...`
  );

  // Listen for the event announcing that the request was fulfilled
  function fulfilled(requestId: string) {
    return new Promise((resolve) =>
